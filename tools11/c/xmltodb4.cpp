...
class CTABCOLS
{
  ...
  char m_allcols[3001];  // 全部字段名列表，以字符串存放，中间用半角的逗号分隔。
...
bool _xmltodb()
{
  ...
  while (true)
  {
    ...
    while (true)
    {
      ...
      {
        logfile.WriteEx("ok.\n");
        // 把xml文件移动到starg.xmlpathbak参数指定的目录中，一般不会发生错误，如果真的发生了，程序将退出。
      ...
      if ((iret==1) || (iret==2))
      {
        ...
        // 把xml文件移动到starg.xmlpatherr参数指定的目录中，一般不会发生错误，如果真的发生了，程序将退出。
...
int _xmltodb(char *fullfilename,char *filename)
{
  // 从vxmltotable容器中查找filenam的入库参数，存放在stxmltotable结构体中。
...
void CTABCOLS::initdata()  // 成员变量初始化。
{
  ...
  m_vallcols.clear();      // 清空容器。
  m_vpkcols.clear();       // 清空容器。
...
void crtsql()
{
  ...
  for (int ii=0;ii<TABCOLS.m_vpkcols.size();ii++)
    for (int jj=0;jj<TABCOLS.m_vallcols.size();ii++)
